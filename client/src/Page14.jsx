...
const Page14 = () => {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          "http://10.10.20.73:8081/api/getgraph_coursecompletionovertime?p_usrid=27"
        );
  ...
  const apexOptions = {
    ...
    xaxis: {
      categories: chartData.lineChart.lessons,
      title: { text: "Lessons", style: { fontSize: "14px", fontWeight: 600 } },
    },
    yaxis: {
      title: {
        text: "Time Taken (s)",
  ...
  const echartOptions = {
    title: {
      text: "Time Taken per Lesson (s)",
      left: "center",
      textStyle: { fontSize: 16, fontWeight: "bold" },
    },
    xAxis: {
      type: "category",
      data: chartData.lineChart.lessons,
    ...
    series: [
      {
        data: chartData.lineChart.timeTaken,
        type: "line",
        smooth: true,
  ...
  const pieData = [
    {
      name: "Completed",
      value: chartData.lineChart.promptsCompleted.reduce((a, b) => a + b, 0),
    },
    {
      name: "Remaining",
      value:
        chartData.lineChart.totalPrompts.reduce((a, b) => a + b, 0) -
        chartData.lineChart.promptsCompleted.reduce((a, b) => a + b, 0),
    },
  ];
        ...
        <div>
          {selectedChart === "ApexCharts" && (
            <ApexCharts
              options={apexOptions}
              series={[
                { name: "Time Taken (s)", data: chartData.lineChart.timeTaken },
              ]}
              type="area"
              height={600}
            />
          ...
          {selectedChart === "ECharts" && (
            <div ref={echartsRef} style={{ height: 600, width: "100%" }} />
          )}
            ...
            <ResponsiveContainer width="100%" height={600}>
              <LineChart data={rechartsData}>
                ...
                <Line
                  ...
                  dataKey="timeTaken"
                  stroke="#FF4560"
                  dot={{ stroke: "#FF4560", strokeWidth: 2 }}
                />
              </LineChart>
            </ResponsiveContainer>
          )}
